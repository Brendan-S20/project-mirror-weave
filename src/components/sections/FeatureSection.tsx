      ...
      <section className="section-padding py-20 lg:py-28" ref={ref}>
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl mb-16">
            {tagline && <span className="text-xs font-semibold uppercase tracking-widest text-primary/80 mb-4 block">{tagline}</span>}
            <h2 className="text-3xl lg:text-5xl font-bold text-foreground mb-4">{title}</h2>
            {description && <p className="text-lg text-muted-foreground">{description}</p>}
          </div>
          <div className={`space-y-8 ${inView ? "stagger-children" : ""}`}>
            {features.map((f, i) => (
              <div key={i} className={`flex gap-6 items-start pb-8 border-b border-border/30 last:border-0 ${inView ? "animate-fade-up" : "opacity-0"}`}>
    ...
    <section className="section-padding py-20 lg:py-28" ref={ref}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className={`grid ${colsClass} gap-6 ${inView ? "stagger-children" : ""}`}>
          {features.map((f, i) => (
            <div key={i} className={`card-elevated p-8 hover:border-primary/20 transition-all duration-300 ${inView ? "animate-fade-up" : "opacity-0"}`}>
